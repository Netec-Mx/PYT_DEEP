# Primera clasificación de Productos Peligrosos Usando Deep Learning

## Objetivo de la práctica:
Al final de la actividad, serás capaz de desarrollar un modelo de redes neuronales que clasifique productos como peligrosos o no peligrosos, utilizando características como peso, valor declarado, país de origen y tipo de producto. Además deberás comprender las métricas de rendimiento para que a lo largo del curso optimices este primer modelo creado.
...
Paso 1. Importar las librerías necesarias para el proyecto:
```python
import pandas as pd
import numpy as np
...
```
Paso 2. Permitir que Colab acceda a la carga del dataset con 5000 muestras y 5 columnas de las cuales la red aprenderá para llegar a una futura predicción:

```python
uploaded = files.upload()
```
Paso 3. Seleccionar la opción de "Elegir archivos"

![imagen resultado](../images/cap1_1.png) 

Paso 4. Seleccionar el archivo "datos_aduana.csv" y dar clic en "Abrir"

![imagen resultado](../images/cap1_2.png)

Paso 5. Convertir el dataset en un DataFrame de Pandas, que es una estructura de datos bidimensional similar a una tabla (filas y columnas). Contiene los siguientes datos:
...
Paso 8. Escalar los valores de `X` a una distribución estándar (media 0, desviación estándar 1) usando `StandardScaler()`. Esto es importante para mejorar el rendimiento de las redes neuronales.
...
Paso 9. Dividir el dataset en un conjunto de entrenamiento (80%) y un conjunto de prueba (20%) con `train_test_split()`.
...
Paso 10. Definir una red neuronal secuencial con varias capas:
  - **Capa densa (32 neuronas)**: La primera capa tendrá 32 neuronas con activación ReLU y recibirá como entrada las características de `X_train`.
  - **Capa de Dropout**: El Dropout (20%) servirá para prevenir el sobreajuste, apagando aleatoriamente un porcentaje de neuronas durante el entrenamiento.
  - **Capa densa (16 neuronas)**: En la segunda capa se definirán 16 neuronas y activación ReLU.
  - **Capa de Dropout**: Nuevamente se usa Dropout del 20%.
  - **Capa densa (8 neuronas)**: Tercera capa con 8 neuronas y activación ReLU.
  - **Capa de salida (1 neurona)**: La última capa usa activación `sigmoid` para producir un valor entre 0 y 1, que representa la probabilidad de que el producto sea peligroso.
...
Paso 12. Entrenar el modelo durante 50 épocas con un tamaño de lote de 64. Se reserva el 20% de los datos de entrenamiento como conjunto de validación para medir el rendimiento del modelo durante el entrenamiento.
...
Paso 13. Guardar y descargar el historial de entrenamiento, que incluye la precisión y la pérdida en cada época en un archivo CSV llamado `historial_entrenamiento.csv`.
...
Paso 14. Evaluar el modelo usando los datos de prueba e imprimir la precisión alcanzada
...
Paso 15. Visualizar la precisión y pérdida del modelo
...
Paso 16. Cargar nuevos datos para hacer predicciones
...
Paso 17. Aplicar **one-hot encoding** a las variables categóricas (país de origen y tipo de producto) para convertirlas en variables numéricas. 
...
Paso 18. Hacer predicciones para los nuevos datos y usar un umbral de 0.5 para clasificar los resultados como **peligroso (1)** o **no peligroso (0)**.
...
Paso 19. Generar un **reporte detallado** para cada uno de los nuevos envíos e incluir la información del envío (país de origen, peso, valor declarado, tipo de producto) y la **predicción del modelo** comparada con la **clasificación real**.
