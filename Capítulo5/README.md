...
Al finalizar la actividad, serás capaz de aplicar técnicas de deep learning para realizar análisis de sentimientos en redes sociales, desarrollando un modelo que identifique y clasifique opiniones de usuarios de aerolíneas en distintas categorías. Aprenderás a preprocesar texto logrando extraer información valiosa a partir de grandes volúmenes de datos textuales.
...
Instalar la biblioteca kaggle, descargar el [conjunto de datos de sentimientos de publicaciones en Twitter relacionado con aerolíneas](https://www.kaggle.com/datasets/crowdflower/twitter-airline-sentiment), y descomprirlos para su uso.
...
Paso 2. Importar las bibliotecas necesarias para procesamiento de datos (pandas, numpy), procesamiento de texto (nltk y re), visualización (matplotlib y seaborn), división de datos y métricas de evaluación (sklearn), y el manejo de redes neuronales recurrentes (tensorflow.keras).
...
Paso 3. Cargar el archivo Tweets.csv en un DataFrame de pandas
...
Paso 4. Descargar los recursos de nltk necesarios para eliminar palabras comunes (stopwords), lematización (wordnet) y tokenización (punkt).
...
Paso 5. Limpiar cada tweet para eliminar enlaces y menciones, caracteres especiales y números, además de convertir el texto a minúsculas y tokenizarlo. También lematizar las palabras para reducirlas a su forma raíz y aplicar esta función a la columna text para crear una nueva columna cleaned_text con los tweets preprocesados.
...
Paso 7. Asignar los textos limpios a X (entrada) y las etiquetas a y (salida).
...
Paso 8. Definir el número máximo de palabras únicas y la longitud máxima de secuencia y crear un Tokenizer para asignar números a las palabras ajustando su longitud para que todas las secuencias tengan la misma longitud.
...
Paso 9. Dividir los datos en conjuntos de entrenamiento y prueba (80% y 20%, respectivamente) manteniendo la distribución de clases.
```python
...
Paso 10. Construir una red neuronal con las siguientes capas:
...
Compilar el modelo utilizando la pérdida categorical_crossentropy y el optimizador adam.
...
Paso 11. Entrenar el modelo con 5 épocas y un tamaño de lote de 64, reservando el 10% del conjunto de entrenamiento para validación.
...
Paso 12. Evaluar el modelo con el conjunto de prueba y mostrar la precisión, el reporte de clasificación y la matriz de confusión.
